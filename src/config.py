...
class Config():
    ...
    def _set_stdout(verbose: bool = True, prompt: str = 'log setup OK -> stdout'):
        logging.basicConfig(
            stream=sys.stdout, 
    ...
    def _set_logger(log_dir: str, log_name: str, verbose: bool = True, prompt: str = 'log setup OK -> logger:'):
    ...
    def __set(self, stdout: bool, dry_run: bool, prompt: str):
        if stdout:
            ...
            Config._set_logger(self.log_dir, self.log_name, verbose=self.verbose)
    ...
    def __init__(self, stdout: bool, dry_run = False, verbose = True) -> None:
        ...
        log_base = os.path.join(os.path.join(Config.LOG_ROOT, f'{date.today()}'))
