...
class Board():
    ...
    def __init__(self, width: float = 370.0, height: float = 370.0, resolution: float = 0.05) -> None:
    ...
    def size(self) -> tuple[int, int]:
        return (int(self.width / self.resolution), int(self.height / self.resolution))
    ...
    def drawLine(self, line: 'Line') -> None:
        ...
        if dx == 0 and dy == 0:
            ...
            for xid in range(0, dx, 1 if dx > 0 else -1):
                self.image.putpixel((x_src + xid, round(xid * slope + x_src)), Board.FILLED)
    ...
    def drawRect(self, rect: Rectangle) -> None:
        x0, y0 = self.pixel(Point(rect.center.x - rect.width / 2, rect.center.y - rect.height / 2))
        x1, y1 = self.pixel(Point(rect.center.x + rect.width / 2, rect.center.y + rect.height / 2))
