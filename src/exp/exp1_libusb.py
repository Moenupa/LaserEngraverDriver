'''
attempt to use usbrply to reverse-engineer the control-code from captured packets.
failed on Feb. 6, 2023

problem: captured packets are good, but usb1 returns error:
`sb1.USBErrorNotSupported: LIBUSB_ERROR_NOT_SUPPORTED` which indicates 
pyusb1 does not support the driver
...
def open_dev(usbcontext=None):
    ...
    for udev in usbcontext.getDeviceList(skip_on_error=False):
        ...
        if (vid, pid) == (0x1a86, 0x7523):
            ...
            pprint(get_object_properties(udev.device_descriptor))
            return udev.open()
